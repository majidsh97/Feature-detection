...
def extract_features(img: t_img, num_features: int = 500) -> Tuple[t_points, t_descriptors]:
    ...
    keypoints , desciptors= orb.detectAndCompute(img, None) # params(img, mask)
    #keypoints cv2.Keypoint.point = (x, y)
    pts = cv2.KeyPoint_convert(keypoints)  #
    
    return pts, desciptors
...
def filter_and_align_descriptors(f1: Tuple[t_points, t_descriptors], f2: Tuple[t_points, t_descriptors],
    ...
    """
    Given the two nearest neighbors for a descriptor src image, a good way of identifying outliers
is to compute the ratio between the distances d0 and d1. If this ratio is above a threshold tr,
this match is considered ambiguous and classified as a descriptor-outlier.
    
    Aligns pairs of keypoints from two images.
    Aligns keypoints from two images based on descriptor similarity.
    If K points have been detected in image1 and J points have been detected in image2, the result will be two sets of N
    ...
    """
    # TODO: check the shape -> descriptor and points sizes
    ...
    # N x J matrix
    diff = spatial.distance.cdist(f1[1], f2[1], metric=similarity_metric)
    ...
    min_loc = np.argmin(diff, axis=1)
    sorted_diffs = np.sort(diff, axis=1)
    nn1 = sorted_diffs[:, 0]
    nn2 = sorted_diffs[:, 1]
    ratios = (nn1 / nn2)
    significant = (ratios <= similarity_threshold)

    # step 4: removing non significant matches and return the aligned points (their location only!)
    src_loc = src_pts[significant]
    valid_dest_indices = min_loc[significant]
    dest_pts = dest_pts[valid_dest_indices]

    return src_loc, dest_pts
...
def compute_homography(f1: np.array, f2: np.array) -> np.array:
    ...
    A = np.vstack([-x, -y, -np.ones_like(x), np.zeros_like(x), np.zeros_like(x), np.zeros_like(x),
                   u * x, u * y, u]).T
    A = np.vstack([A, np.vstack([np.zeros_like(x),np.zeros_like(x),np.zeros_like(x),
                                 -x, -y, -np.ones_like(x), v * x, v *y , v]).T])
...
def _get_inlier_count(src_points: np.array, dst_points: np.array, homography: np.array,
    ...
    norm_src_pts = np.hstack([src_points , np.ones((src_points.shape[0], 1))])
    # norm_dst_pts = np.hstack([dst_points , np.ones((dst_points.shape[0], 1))])
    # step 2: project the image points from image 1 to image 2 using the homography (1 line)
    # Hint: You can use np.dot here
    projection = norm_src_pts @ homography.T
    # step 3: re-normalize the projected points ([x, y, l] --> [x/l, y/l]) (1 line)
    projected_src = projection[:,0:2] / projection[:,2, np.newaxis]

    # step 4: compute and return number of inliers (3 lines)
    # Hint: You might use np.linalg.norm
    diffs= spatial.distance.cdist(projected_src,dst_points)

    num_inliers = np.sum(diffs < distance_threshold)

    return num_inliers


def ransac(src_features: Tuple[t_points, t_descriptors], dst_features: Tuple[t_points, t_descriptors], steps,
           distance_threshold, n_points=9, similarity_threshold=.7) -> np.array:
    ...
    src, dst= filter_and_align_descriptors(src_features, dst_features, similarity_threshold= similarity_threshold)
    ...
    for n in range(steps):
        ...
        subset_rand_indices = np.random.choice(src.shape[0], size=n_points) #replace : donn't select val twice
        src_subset = src[subset_rand_indices]
        dst_subset = dst[subset_rand_indices]
        ...
        curr_inlier_num = _get_inlier_count(src,dst, H, distance_threshold=distance_threshold)
        if curr_inlier_num > best_count:
            ##update H
            best_homography = H
            best_count = curr_inlier_num
...
def probagate_homographies(homographies: t_homographies, reference_name: str) -> t_homographies:
    ...
    Args:
        ...
        reference_name: The name of the image which will be the destination for all homographies.
...
def translate_homographies(homographies: t_homographies, dx: float, dy: float):
    ...
    for key, homography in homographies.items():
        ...
        translated_homographies[key] = translated_homography
...
def stitch_panorama(images: t_images, homographies: t_homographies, output_size: Tuple[int, int],
    ...
    Args:
        ...
        rendering_order: A list containing the names of the images representing the order in which the images will be
